...
void	set_add(const t_vec *self, const t_vec *u)
{
	self->x += u->x;
	self->y += u->y;
	self->z += u->z;
...
void	set_subt(const t_vec *self, const t_vec *u)
{
	self->x -= u->x;
	self->y -= u->y;
	self->z -= u->z;
...
void	set_mul(const t_vec *self, const t_vec *u)
{
	...
	if (self->type == T_VEC && u->type == T_DOUBLE)
	{
		self->x *= u->num;
		self->y *= u->num;
		self->z *= u->num;
...
t_vec	vec_mul(const t_vec *u, const t_vec *v)
{
	...
	if (u->type == T_DOUBLE && v->type == T_VEC)
		return (vec_mul_double_vec(u->num, &v->vec));
	if (u->type == T_VEC && v->type == T_DOUBLE)
		return (vec_mul_double_vec(v->num, &u->vec));
	return (vec_init());
}

t_vec	vec_dev(const t_vec *u, const double t)
{
	t_vec	d_num;
